...
public class CmdServiceImpl {
	...
	public Data getLastNLineLog(long prevSize) throws IOException {
		List<String> result = new ArrayList<>();
		RandomAccessFile randomAccessFile = new RandomAccessFile(Constants.FILE_DIR, "r");

		long currLen = randomAccessFile.length();

		if (prevSize == currLen) {
			// No updated happened.
			return new Data(new ArrayList<>(), currLen);
		}
		...
		while (readMore) {
			...
			readMore = result.size() < (currLen - prevSize) && startPoint != 0;

		}
		return new Data(result, currLen);
	}
